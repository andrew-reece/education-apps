...
if ($step == 1) {
	...
	$table = "temp_register";
	$field_names = "confirmation_code, username, password, email, study, pimindcre_group, step";
	$field_values = "'$validate_code', '$username', '$password', '$email', '$study', '$this_group_num', '1'";
	...
	$db_names = "username, password, email, birthdate, gender, race, hand, artistic, instruments, profession, ed_level, discipline, highest_ed";
	$db_values = array($username,$md5_password, $email, $birthdate, $gender, $race, $hand, $artist, $instruments, $profession, $edinfo_degrees, $edinfo_major, $highest_degree);
	
	$this_study_name = getStudyNickname($study);

	// HERE WE PASS INFO FOR BOTH participants AND study_members DATA
	// WE NEED TO EXTRACT MORE INFO IN ORDER TO SUCCESSFULLY POPULATE THE study_members DB, SEE reg_functions.php FOR MORE
	$db2_names = "id, $this_study_name, pimindcre_group";
	$uid = registerUser($db_names, $db_values, $db2_names, $this_study_name, $pimindcre_group);
	deleteTempData($key);
